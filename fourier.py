...
class SpectralConv2d(nnx.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2, rngs: nnx.Rngs):
        ...
        self.modes1 = modes1 # number of lowest Fourier x-frequencies to keep; higher frequencies are discarded
        self.modes2 = modes2 # number of lowest Fourier y-frequencies to keep; higher frequencies are discarded
    ...
    def __call__(self, x):
        ...
        out_ft = jnp.zeros((batch, self.out_channels, h, w // 2 + 1), dtype=jnp.complex64) # blank frequency domain canvas
...
class FNO2d(nnx.Module):   # high-level architecture to stack several Spectral Convolutions layers
    ...
    def __call__(self, x):   # make FNO2d class a function
        ...
        x = self.fc1(x4)   # final projection
        x = jax.nn.gelu(x)
        x = self.fc2(x)
        return x
